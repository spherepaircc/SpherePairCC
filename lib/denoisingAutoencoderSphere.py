...
class DenoisingAutoencoder(nn.Module):
    def __init__(self, in_features, out_features, activation="relu", 
        dropout=0.2, tied=False, normalize=False):
        ...
        self.normalize = normalize     # Sphere: need to normalize the latent representation
    ...
    def forward(self, x):
        if self.enc_act_func is not None:
            h = self.dropout(self.enc_act_func(F.linear(x, self.weight, self.bias)))
        else:
            h = self.dropout(F.linear(x, self.weight, self.bias))
        if self.normalize:
